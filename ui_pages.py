# ============================================================
#  ui_pages.py  —  Genki Beibei v2.2
#  ------------------------------------------------------------
#  包含 Stroop 與 PVT-B 雙模式選擇，移除冗餘防呆機制
# ============================================================
...
def show_analyzer() -> None:
    ...
    st.markdown("---")
    next_label = "Stroop 干擾測驗" if st.session_state.test_mode == "stroop" else "PVT-B 反應時間測驗"
...
def show_pvt_game() -> None:
    ...
    # 純淨版 JS：測驗結束後直接觸發 window.parent.location.href 跳轉
    stroop_html = f"""
<!DOCTYPE html>
<html>
<head>
...
<script>
  ...
  function recordResponse(correct, rt) {{
    if (rt < FALSE_START) {{ falseStarts++; return; }}  
    if (!correct) {{ falseStarts++; return; }}            
